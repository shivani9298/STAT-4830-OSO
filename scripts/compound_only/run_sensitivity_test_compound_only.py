...
def main() -> int:
    ...
    # Compound-growth only objective:
    # L = -mean(log(1+r)); all other terms disabled.
    ...
    csv_path = out_dir / "sensitivity_test_compound_only.csv"
    res_df.to_csv(csv_path, index=False)

    summary = {
        "objective": "compound_growth_only",
        "n_tests": int(len(res_df)),
        "top_by_mean_abs_delta_ipo_weight": res_df.head(10).to_dict(orient="records"),
    ...
    json_path = out_dir / "sensitivity_test_compound_only_summary.json"
    json_path.write_text(json.dumps(summary, indent=2))

    topn = res_df.head(12).copy()
    fig, ax = plt.subplots(figsize=(10.5, 5.8))
    ax.barh(topn["test"][::-1], topn["mean_abs_delta_ipo_weight"][::-1], color="C1")
    ax.set_title("Sensitivity test (compound-growth-only): mean |delta IPO weight|")
    ax.set_xlabel("Mean absolute IPO-weight change")
    ax.grid(True, axis="x", alpha=0.3)
    fig.tight_layout()
    bar_path = fig_dir / "sensitivity_compound_only_top_mean_abs_delta_ipo_weight.png"
