...
LOOKBACK_DAYS = 126  # show ~6 months before validation starts
...
# Optional full-period weights exported by run_ipo_optimizer_wrds.py.
# If present, we can extend the "validation objective" chart backward.
weights_full_path = ROOT / "results" / "ipo_optimizer_weights_full.csv"
weights_full = None
if weights_full_path.exists():
    weights_full = pd.read_csv(weights_full_path, index_col="date", parse_dates=True)
else:
    # Backward-compatible fallback: older runs store full-period model weights
    # in ipo_180day_mcap_weights.csv with columns SPY/IPO_INDEX.
    legacy_weights_path = ROOT / "results" / "ipo_180day_mcap_weights.csv"
    if legacy_weights_path.exists():
        weights_full = pd.read_csv(legacy_weights_path, index_col=0, parse_dates=True)
        rename_map = {}
        if "SPY" in weights_full.columns:
            rename_map["SPY"] = "weight_market"
        if "IPO_INDEX" in weights_full.columns:
            rename_map["IPO_INDEX"] = "weight_IPO"
        weights_full = weights_full.rename(columns=rename_map)
        keep_cols = [c for c in ["weight_market", "weight_IPO"] if c in weights_full.columns]
        if len(keep_cols) == 2:
            weights_full = weights_full[keep_cols]
        else:
            weights_full = None
...
# Build an extended objective series that reaches back into pre-validation data
# when full-period weights are available.
if weights_full is not None:
    ret_full = mcap[["SPY_Only", "IPO_Only"]].reindex(weights_full.index).dropna()
    wts_full = weights_full.reindex(ret_full.index)

    val_start = dates[0]
    val_end = dates[-1]
    lookback_start = val_start - pd.Timedelta(days=LOOKBACK_DAYS)
    ext_mask = (ret_full.index >= lookback_start) & (ret_full.index <= val_end)

    ret_obj = ret_full.loc[ext_mask]
    wts_obj = wts_full.loc[ext_mask]
    w_m_obj = wts_obj["weight_market"].values
    w_i_obj = wts_obj["weight_IPO"].values
    model_ret_obj = w_m_obj * ret_obj["SPY_Only"].values + w_i_obj * ret_obj["IPO_Only"].values
    obj_dates = ret_obj.index
    obj_ext, mean_ext, vol_ext, cvar_ext, vexc_ext = rolling_objective(
        model_ret_obj, w_m_obj, w_i_obj, win=WINDOW
    )
else:
    val_start = dates[0]
    obj_dates = dates
    obj_ext, mean_ext, vol_ext, cvar_ext, vexc_ext = obj, comp_mean, comp_vol, comp_cvar, comp_vexc

fig2, axes2 = plt.subplots(2, 1, figsize=(10, 7), sharex=True)

ax = axes2[0]
ax.plot(obj_dates, obj_ext, color="#d62728", lw=1.5, label="Combined objective L")
ax.axvline(val_start, color="#7f7f7f", lw=1.0, ls="--", label="Validation starts")
ax.axhline(0, color="k", lw=0.6, ls=":")
ax.set_ylabel("Objective value (lower = better)")
if weights_full is not None:
    ax.set_title(
        f"Rolling Optimization Objective ({WINDOW}-day window) — "
        f"Validation + {LOOKBACK_DAYS}-day Training Lookback"
    )
else:
...
ax = axes2[1]
ax.plot(obj_dates, mean_ext, color="#1f77b4", lw=1.2, label="Mean return (reward)")
ax.plot(obj_dates, -cvar_ext, color="#e377c2", lw=1.2, label="-CVaR penalty")
ax.plot(obj_dates, vol_ext,   color="#ff7f0e", lw=1.2, label="Variance penalty")
ax.plot(obj_dates, vexc_ext,  color="#bcbd22", lw=1.2, label="Vol-excess penalty")
ax.axvline(val_start, color="#7f7f7f", lw=1.0, ls="--")
ax.axhline(0, color="k", lw=0.6, ls=":")
