...
def run_training_online_step(
    ...
    *,
    ...
    batch_size: int = 128,
    patience: int = 3,
    stop_on_val_deterioration: bool = False,
    deterioration_tol: float = 0.0,
    warm_start: bool = True,
    reset_optimizer_on_update: bool = False,
    lambda_cvar: float = 0.5,
    lambda_turnover: float = 0.01,
    lambda_vol: float = 0.5,
    lambda_path: float = 0.01,
    ...
    verbose: bool = False,
    log_every: int = 0,
    log_loss_term_breakdown: bool = False,
) -> tuple[torch.nn.Module, torch.optim.Optimizer, list[dict]]:
    """
    One online update step (warm-start friendly).

    ``data_step`` expects ``X_train``, ``R_train`` and optional ``X_val``, ``R_val``.
    Returns updated ``(model, optimizer, history_rows)``.
    """
    if device is None:
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    X_train = _to_float_tensor(data_step["X_train"])
    R_train = _to_float_tensor(data_step["R_train"])
    X_val_in = data_step.get("X_val", None)
    R_val_in = data_step.get("R_val", None)
    if X_val_in is None or R_val_in is None:
        X_val = X_train
        R_val = R_train
    else:
        X_val = _to_float_tensor(X_val_in)
        R_val = _to_float_tensor(R_val_in)
        if X_val.shape[0] == 0:
            X_val = X_train
            R_val = R_train

    if X_train.shape[0] == 0:
        raise ValueError("run_training_online_step requires non-empty X_train")

    n_features = X_train.shape[2]
    seq_len = X_train.shape[1]
    n_assets = int(data_step.get("n_assets", R_train.shape[-1]))
    ...
    if optimizer is None or reset_optimizer_on_update:
        optimizer = torch.optim.Adam(model.parameters(), lr=lr, weight_decay=weight_decay)
    else:
    ...
    sched_name = _resolve_lr_schedule(lr_schedule, False)
    scheduler = _build_lr_scheduler(
        optimizer,
        sched_name,
        max(1, int(epochs_step)),
        plateau_factor=lr_decay,
        plateau_patience=plateau_patience,
        min_lr=min_lr,
        exponential_gamma=exponential_gamma,
    ...
    for epoch in range(max(1, int(epochs_step))):
        train_loss, train_comp, train_diag = train_epoch(
            model, X_train, R_train, optimizer, device,
            batch_size=batch_size,
            lambda_cvar=lambda_cvar,
        ...
        if verbose and log_every > 0:
            ep = epoch + 1
            if ep == 1 or ep % log_every == 0 or ep == epochs_step:
                print(
                    f"[IPO-online] epoch {ep}/{epochs_step} train={train_loss:.6f} val={val_loss:.6f}",
                    flush=True,
                )
                if log_loss_term_breakdown and train_tb:
                    print(_format_term_breakdown_table(train_tb, "train"), flush=True)
        ...
        if epochs_no_improve >= max(1, int(patience)):
            break
        if (
            stop_on_val_deterioration
            and prev_val_loss is not None
            and float(val_loss) > float(prev_val_loss) + float(deterioration_tol)
        ):
            break
        prev_val_loss = float(val_loss)
