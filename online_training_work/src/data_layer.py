...
UpdateFreq = Literal["D", "W", "M"]


def _normalize_update_freq(update_freq: str) -> UpdateFreq:
    f = str(update_freq).strip().upper()
    aliases = {
        "DAILY": "D",
        "DAY": "D",
        "D": "D",
    ...
    if f not in aliases:
        raise ValueError("update_freq must be one of D/W/M (or daily/weekly/monthly)")
    return aliases[f]
...
def build_online_schedule(
    ...
    embargo_windows: int = 0,
) -> list[dict[str, Any]]:
    """
    Build an expanding-window online schedule over rolling-window rows.

    Each row in the returned schedule corresponds to one policy decision and contains:
    - ``decision_idx``: window-row index used for action generation.
    - ``eval_idx``: window-row index whose realized return is scored (``decision_idx + decision_lag``).
    - ``train_end_idx``: last window-row index allowed for training (strictly before ``decision_idx``).
    - ``is_update_step``: whether to run a model update at this decision.
    """
    ...
    if use_embargo and embargo_windows < 0:
        raise ValueError("embargo_windows must be >= 0")

    freq = _normalize_update_freq(update_freq)
    first_decision_idx = int(warmup_windows)
    if first_decision_idx >= n:
        return []

    schedule: list[dict[str, Any]] = []
    should_update = True
    last_period: tuple[int, int] | int | None = None
    for decision_idx in range(first_decision_idx, n, int(step)):
        eval_idx = decision_idx + int(decision_lag)
        if eval_idx >= n:
            break

        train_end_idx = decision_idx - 1
        if use_embargo and embargo_windows > 0:
            train_end_idx = decision_idx - 1 - int(embargo_windows)
        if train_end_idx < 0:
            continue

        ts = pd.Timestamp(d[decision_idx])
        if freq == "D":
            is_update_step = should_update
        elif freq == "W":
            period_key = (ts.isocalendar().year, ts.isocalendar().week)
            is_update_step = period_key != last_period
        ...
        schedule.append(
            {
                ...
                "decision_date": ts,
                "eval_date": pd.Timestamp(d[eval_idx]),
            }
        )
        should_update = False
    return schedule
...
def slice_windows_by_index(
    ...
    """
    Slice rolling-window arrays up to ``end_idx`` (inclusive).

    If ``lookback_windows`` is provided, keep at most that many windows ending at ``end_idx``.
    """
    n = len(dates)
    if n == 0:
        return X[:0], R[:0], dates[:0]
    e = int(end_idx)
    if e < 0:
        return X[:0], R[:0], dates[:0]
    if e >= n:
    ...
    if lookback_windows is None or int(lookback_windows) <= 0:
        s = 0
    else:
        s = max(0, e - int(lookback_windows) + 1)
    return X[s : e + 1], R[s : e + 1], dates[s : e + 1]
