...
def static_baseline_weights(n_days: int, n_assets: int) -> dict[str, np.ndarray]:
    """
    Fixed weight matrices (N, K) on the same calendar as R for apples-to-apples baselines.
    Column 0 is treated as the broad market sleeve; remaining columns are sector/IPO sleeves.
    """
    k = n_assets
    out: dict[str, np.ndarray] = {}
    out["equal_1/K"] = np.full((n_days, k), 1.0 / k, dtype=np.float64)
    w_m = np.zeros((n_days, k), dtype=np.float64)
    w_m[:, 0] = 1.0
    out["market_only"] = w_m
    if k >= 2:
        w_nm = np.zeros((n_days, k), dtype=np.float64)
        w_nm[:, 1:] = 1.0 / (k - 1)
        out["non_market_equal"] = w_nm
    return out


def baseline_stats_same_R(R: np.ndarray) -> list[tuple[str, dict]]:
    """Static benchmarks on the same realized returns matrix R (N, K) as the model."""
    n, k = R.shape
    baselines = static_baseline_weights(n, k)
    labels = {
        "equal_1/K": "Equal weight 1/K (all sleeves)",
        "market_only": "Market only (100% sleeve 0)",
        "non_market_equal": (
            "IPO/sleeves equal (0% market, 1/(K-1) each other)"
            if k > 2
            else "IPO only (100% sleeve 1)"
        ),
    }
    return [(labels[key], portfolio_stats(w, R)) for key, w in baselines.items()]
