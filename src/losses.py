...
def loss_excess_vs_equal_weight(returns: torch.Tensor, port_ret: torch.Tensor) -> torch.Tensor:
    """
    Penalize underperformance vs the fixed equal-weight portfolio (1/K each asset).

    Equal-weight return per row: mean_i r_i. Minimizing L = -(port - ew).mean()
    maximizes mean excess return over 1/K (same R column as the learned portfolio).
    """
...
def combined_loss(
    ...
    lambda_vs_ew: reward beating the equal-weight (1/K) portfolio on the same batch returns.
    """
