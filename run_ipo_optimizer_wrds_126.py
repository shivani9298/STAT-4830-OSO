...
def main():
    ...
    # Also export full-period weights (train + validation windows) so downstream
    # analysis can extend charts backward into the pre-validation regime.
    weights_full = predict_weights(model, X, device)
    stats = portfolio_stats(weights, data["R_val"])

    out_dir = ROOT / "results"
    out_dir.mkdir(exist_ok=True)
    weights_path = out_dir / "ipo_optimizer_weights.csv"
    weights_full_path = out_dir / "ipo_optimizer_weights_full.csv"
    summary_path = out_dir / "ipo_optimizer_summary.txt"

    export_weights_csv(data["dates_val"], weights, weights_path)
    export_weights_csv(dates, weights_full, weights_full_path)
    export_summary(stats, weights, summary_path, R=data["R_val"])
    print(f"Exported weights to {weights_path}")
    print(f"Exported full-period weights to {weights_full_path}")
